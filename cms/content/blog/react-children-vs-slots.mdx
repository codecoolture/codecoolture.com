...
populate inner pieces of UI has been a prevalent pattern in the React community,
...
The example above extensively uses the `children` prop and composition to render
the desired UI. Instead of passing multiple props to the `Nav` component, we
create many small, dedicated components to render each of the inner pieces of
UI. This approach gives us a much more flexible and idiomatic API. It makes it
possible to add additional elements to the `Nav` component without changing the
API (following the [Open-Closed design principle][open-closed]).

On the other hand, this approach has some drawbacks worth mentioning:
...
- A few weeks after starting to write this post, I received a newsletter from
  ...
  content of [the article][soul-crushing-components] covered precisely this! So,
  digging a little more, I found this pattern had been documented since a few
  years ago (watch out [this video talk][talk] from Ryan Florence at Phoenix
  ReactJS conf in 2017), and it is called _Compound components_.
