...
describe("POST /mechanics/{id}/cars", () => {
  ...
  it("does not add a new car if the mechanic is working on five cars already", async () => {
    ...
    const car6 = new Car({ id: 6, make: "Mazda", model: "CX-9", year: 2021 });
    await carRepository.save(car6);

    const response = await request(app).post(`/mechanics/${mechanic.id}/cars`).send({ carId: car6.id });
